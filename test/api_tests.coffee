Meteor.startup ->
  ...
  describe 'An API route', ->
    it 'should use the default OPTIONS endpoint if none is defined for the requested method', (test, next) ->
      ...
      HTTP.call 'OPTIONS', Meteor.absoluteUrl('api/v1/default-endpoints'), next (error, result) ->
  ...
  describe 'An API collection route', ->
    it 'should be able to exclude endpoints using just the excludedEndpoints option', (test, next) ->
      ...
      HTTP.get Meteor.absoluteUrl('api/v1/excluded-endpoints/10'), next (error, result) ->
      ...
      HTTP.get Meteor.absoluteUrl('api/v1/excluded-endpoints/'), next (error, result) ->
        response = JSON.parse result.content
        test.isTrue error
        test.equal result.statusCode, 405
        test.equal response.status, 'error'
      ...
      HTTP.post Meteor.absoluteUrl('api/v1/excluded-endpoints/'), {data: test: 'abc'}, next (error, result) ->
        response = JSON.parse result.content
    ...
    describe 'with the default autogenerated endpoints', ->
      ...
      it 'should not support a DELETE on api/collection', (test, next) ->
        HTTP.del Meteor.absoluteUrl('api/v1/autogen'), next (error, result) ->
          response = JSON.parse result.content
          test.isTrue error
          test.equal result.statusCode, 405
  ...
  describe 'An API endpoint', ->
    ...
    it 'should have access to multiple query params', (test, next) ->
      ...
      HTTP.get Meteor.absoluteUrl('api/v1/mult-query-params?key1=1234&key2=abcd&key3=a1b2'), next (error, result) ->
        test.isTrue result
    ...
    it 'should return a 405 error if that method is not implemented on the route', (test, next) ->
      ...
      HTTP.get Meteor.absoluteUrl('api/v1/method-not-implemented/'), next (error, result) ->
        response = JSON.parse result.content
        test.isTrue error
        test.equal result.statusCode, 405
        test.isTrue result.headers['allow'].indexOf('POST') != -1
      ...
      HTTP.get Meteor.absoluteUrl('api/v1/method-not-implemented/10'), next (error, result) ->
        response = JSON.parse result.content
        test.isTrue error
        test.equal result.statusCode, 405
    ...
    it 'should cause an error when it returns null', (test, next) ->
      ...
      HTTP.get Meteor.absoluteUrl('api/v1/null-response'), next (error, result) ->
        test.isTrue error
    ...
    it 'should cause an error when it returns undefined', (test, next) ->
      ...
      HTTP.get Meteor.absoluteUrl('api/v1/undefined-response'), next (error, result) ->
        test.isTrue error
        test.equal result.statusCode, 500
    ...
    it 'should be able to handle it\'s response manually', (test, next) ->
      ...
      HTTP.get Meteor.absoluteUrl('api/v1/manual-response'), next (error, result) ->
        response = result.content
    ...
    it 'should not have to call this.response.end() when handling the response manually', (test, next) ->
      ...
      HTTP.get Meteor.absoluteUrl('api/v1/manual-response-no-end'), next (error, result) ->
        response = result.content
    ...
    it 'should be able to send it\'s response in chunks', (test, next) ->
      ...
      HTTP.get Meteor.absoluteUrl('api/v1/chunked-response'), next (error, result) ->
        response = result.content
...
#          @response.write 'Testing'
#
#      HTTP.get Meteor.absoluteUrl('api/v1/manual-response-without-done'), next (error, result) ->
#        test.isTrue error
#        test.equal result.statusCode, 500
    ...
    it 'should not wrap text with quotes when response Content-Type is text/plain', (test, next) ->
      ...
      HTTP.get Meteor.absoluteUrl('api/v1/plain-text-response'), next (error, result) ->
        response = result.content
        test.equal result.statusCode, 200
